const {createBoundingBox, isChefInBounds, filterChefByCuisine} = require('./utils');
...
const findChefsByStyle = async (_parent, {geoCoords, cuisineType}, {models}) => {
  const {User} = models;
  const boundingBox = createBoundingBox(geoCoords);
  const errorMsg = 'There are currently no chefs within your location.';
  ...
  try {
    ...
    if (foundChefs.length) {
      ...
      const filteredByCuisine = inBoundChefs.filter(chef => filterChefByCuisine(chef, cuisineType));

      console.log('filteredByCuisine is ', filteredByCuisine)

      if(!filteredByCuisine.length) {
        throw new Error("There were no chefs with that cuisine type in your area.");
      }
